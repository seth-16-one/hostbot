...
async function requestPairingSession(deployment) {
  ...
  if (!baseUrl) {
    throw new Error("PAIRING_SERVER_URL is not configured");
  }
  ...
  const response = await fetch(`${baseUrl.replace(/\/$/, "")}/session`, {
    ...
    headers: {
      ...
      ...(process.env.PAIRING_SERVER_TOKEN
        ? { Authorization: `Bearer ${process.env.PAIRING_SERVER_TOKEN}` }
        : {}),
    },
    body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(payload?.message || "Pairing server request failed");
  }

  return {
    deploymentId: payload.deploymentId || deployment.id,
    pairingCode: payload.pairingCode || payload.pairCode,
    qr: payload.qr || payload.qrValue,
    expiresAt: payload.expiresAt,
    status: payload.status || "pairing",
  };
}
...
app.post("/api/deployments/:id/session", requireAuth, async (req, res) => {
  const marked = updateDb((db) => {
    const deployment = db.deployments.find(
      (item) => item.id === req.params.id && item.userId === req.user.id,
    );
    if (!deployment) return { error: "Deployment not found" };
    deployment.status = "creating_session";
    deployment.updatedAt = new Date().toISOString();
    return { deployment };
  });
  if (marked.error) return res.status(404).json({ message: marked.error });

  try {
    const session = await requestPairingSession(marked.deployment);
    updateDb((db) => {
      const deployment = db.deployments.find(
        (item) => item.id === req.params.id && item.userId === req.user.id,
      );
      if (deployment) {
        deployment.status = session.status;
        deployment.session = {
          id: id("ses"),
          deploymentId: session.deploymentId,
          pairCode: session.pairingCode,
          qrValue: session.qr,
    ...
    res.status(502).json({ message: error.message || "Failed to create pairing session" });
  }
});

app.post("/api/deployments/:id/start", requireAuth, (req, res) => {
  const result = setDeploymentStatus(req.params.id, req.user.id, "deploying");
  if (result.error) return res.status(404).json({ message: result.error });
  res.json({ status: result.deployment.status, deployment: result.deployment });
});
...
app.delete("/api/deployments/:id", requireAuth, (req, res) => {
  const result = updateDb((db) => {
    const index = db.deployments.findIndex(
      (item) => item.id === req.params.id && item.userId === req.user.id,
    );
    if (index === -1) return { error: "Deployment not found" };
